...
describe('isEventVisible', () => {
  ...
  it('ENTITY_DAMAGED: visible if position is visible', () => {
    ...
    expect(isEventVisible({ type: 'ENTITY_DAMAGED', targetId: 'e1', damage: 5, damageType: 'blunt', position: { x: 1, y: 1 } }, state)).toBe(true);
    expect(isEventVisible({ type: 'ENTITY_DAMAGED', targetId: 'e1', damage: 5, damageType: 'blunt', position: { x: 0, y: 0 } }, state)).toBe(false);
  });
...
describe('filterByFOV', () => {
  ...
  it('lifts visible children of invisible parents', () => {
    ...
    const child = makeNode({ type: 'ENTITY_DAMAGED', targetId: 'player', damage: 5, damageType: 'blunt', position: { x: 1, y: 1 } });
