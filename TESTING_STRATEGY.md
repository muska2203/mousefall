...
| `serialization.ts` | Round-trip serialize/deserialize, миграция версий |
...
| Save/load cycle | Save state → load → состояние идентично |
...
│   └── (запланированы)
└── fixtures/
    └── gameState.ts       # Предсобранные состояния для тестов
```
...
| `simulation/serialization.ts` | 90%+ |
...
- ❌ `tests/unit/simulation/serialization.test.ts` — 6 из 9 падают
  (причина: `serialization.ts` полностью закомментирован)

### Проблемы фикстур

`tests/fixtures/gameState.ts` создаёт объекты с устаревим полем `aiState`
вместо актуального `aiStrategy`. Не влияет на movement-тесты, но делает фикстуры несовместимыми с типом `EnemyEntity` при строгой проверке.
