...
export type RelicEffectViewModel = {
  ...
  key: string;
  /** Однострочный текст эффекта: описание правила (может содержать тег-ссылки) или строка модификатора вида «Имя: +N». */
  text: string;
  /** Полярность эффекта для цветового выделения (по образцу свойств карточки экипировки). */
  polarity: 'positive' | 'negative';
};
...
export type RelicChoiceOptionViewModel = {
  ...
  /** Редкость реликвии. */
  rarity: string;
  /** Путь к рамке редкости (для спрайта карточки и поповера). */
  frameUrl: string;
  /** Атмосферный текст (опционально). */
  flavorText?: string;
  /** Эффекты реликвии: сначала правила, затем модификаторы характеристик. */
...
export type RenderInput = {
   ...
   * линия к цели от `attackOrigin` (либо от игрока, если attackOrigin = null).
   * null, если под курсором нет видимого врага.
   */
  enemyHoverOverlay: {
    /** Клетки зоны досягаемости текущего оружия от позиции игрока (тусклая подсветка). */
    rangeCells: Position[];
    /** Клетка врага-цели (подсветка + пунктирная линия к ней). */
    target: Position;
    /** True, если цель уже в зоне поражения: клик бьёт сразу, автопуть не строится. */
    inRange: boolean;
    /**
     * Клетка, из которой будет выполнена атака: последний шаг автопути,
     * построенного именно к этой цели (когда цель вне зоны поражения).
     * null — линия рисуется от игрока (цель в зоне поражения, пути нет,
     * либо путь ведёт к другой цели / в клетку самого врага).
     */
    attackOrigin: Position | null;
  } | null;
