      ...
      for (let i = 0; i < map.corridors.length; i++) {
        ...
        for (const segment of corridor.segments) {
          ...
          if (x1 === x2) {
            ...
            corridorGraphics.rect(x1 - TILE_SIZE / 4, minY - TILE_HEIGHT / 4, TILE_SIZE / 2, maxY - minY + TILE_HEIGHT / 2);
          } else {
            const minX = Math.min(x1, x2);
            const maxX = Math.max(x1, x2);
            corridorGraphics.rect(minX - TILE_SIZE / 4, y1 - TILE_HEIGHT / 4, maxX - minX + TILE_SIZE / 2, TILE_HEIGHT / 2);
          }
