...
/** Параметры исполнителя способности вида «рассекающий удар» (соответствуют полям шаблона kind 'cleave'). */
export interface CleaveSkillParams {
  /** Контентный id способности (шаблона). */
  id: string;
}

/**
 * Тег урона по умолчанию для вида «рассекающий удар».
 * Используется, когда шаблон не задаёт damageTag (урон оружейный).
 */
...
/**
 * Фабрика исполнителя способности вида «рассекающий удар»:
 * удар оружием по дуге из трёх клеток перед кастером (целевая + две боковые).
 *
 * Урон — ролл экипированного оружия с весом по тегу урона (механика
 * оружейного скилла, параметром шаблона не является).
 *
 * Сборку и кэширование выполняет getSkillExecutor.
 */
export function createCleaveSkill(params: CleaveSkillParams): SkillExecutor {
  return {
    id: params.id,
    ...
    resolve(state: GameState, caster: Entity, targets: Position[]): Intent[] {
      ...
      const skillTag = (getSkillDamageTag(ability) ?? CLEAVE_DAMAGE_TAG) as GameplayTag;
