...
/** Радиус разлива масла из фляги. 0 — только клетка цели. */
const OIL_FLASK_RADIUS = 1;

export const oilFlaskSkill: SkillExecutor = {
  id: 'oil_flask',
  ...
  getAffectedPositions(state: GameState, _caster: Entity, _selectedTargets: Position[], hoveredTarget: Position | null): Position[] {
    if (!hoveredTarget) return [];
    return getPositionsInRadius(state, hoveredTarget, OIL_FLASK_RADIUS)
  ...
  resolve(state: GameState, _caster: Entity, targets: Position[]): Intent[] {
    ...
    return getPositionsInRadius(state, target, OIL_FLASK_RADIUS)
      .filter(pos => state.map.tiles[pos.y]?.[pos.x] === 'floor')
      .map(pos => ({
