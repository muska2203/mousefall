...
/** Параметры исполнителя способности вида «рывок» (соответствуют полям шаблона kind 'dash'). */
export interface DashSkillParams {
  /** Контентный id способности (шаблона). */
  id: string;
  /** Расстояние рывка в клетках. */
  distance: number;
  /** Фиксированный урон при столкновении рывка с актором. */
  bumpDamage: number;
}
 ...
 *
 * Параметры механики приходят из шаблона способности (kind 'dash'),
 * сборку и кэширование выполняет getSkillExecutor.
 */
export function createDashSkill(params: DashSkillParams): SkillExecutor {
  /**
  ...
  function getDashTargetPositions(state: GameState, caster: Entity): Position[] {
    ...
    for (const dir of DIRECTIONS) {
      ...
      for (let step = 1; step <= params.distance; step++) {
   ...
   * Клетка на меньшем расстоянии воспринимается как указание направления,
  ...
  function normalizeDashTarget(caster: Entity, target: Position): Position {
    ...
    return {
      x: caster.x + stepX * params.distance,
      y: caster.y + stepY * params.distance,
    };
  }
   ...
   * клетку на расстоянии 1, она нормализуется до клетки на максимальном
   * расстоянии в том же направлении. Движение разбивается на пошаговые
   * MOVE-интенты, чтобы закрытая дверь открывалась перед попыткой встать
   * на её клетку.
   *
   * Порядок интентов:
  ...
  return {
    ...
    getTargetMode(): TargetMode {
      return { type: 'single', range: params.distance };
    },
