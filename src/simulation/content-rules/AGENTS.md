...
- **Порядок слоёв:** `source` → `target` → `world` → `radius` → `global`; внутри `world` — `global` → `tileEffect` → `tileEffectStatus` → `object` → `tileIntrinsic`, затем `priority`, затем `ruleId`.
...
- **Слой `global`:** правило с `reach: 'global'` собирается от всех живых акторов независимо от дистанции (последний слой, дедуп по `(rule.id, selfId)` против `source`/`target`/`radius`). Для on-death/on-remove правил реликвий; обязательно сочетать с `eventRole`/точными условиями. На модификаторы интентов (`apply-intent-modifiers.ts`) слой не распространяется.
...
- **Атрибуция убийства:** `ENTITY_DIED` несёт `sourceEntityId` (источник последнего урона; `null` — смерть от среды). Дот-килл засчитывается самой жертве (тики статусов принадлежат ей), поэтому правила «при убийстве» посторонних не срабатывают — см. `CONTENT_RULES_EDGE_CASES.md`, раздел «Атрибуция убийства».
- **`restoreAp` с `amount`:** эффект `restoreAp` принимает опциональный `amount` (ParametrizedValue) — частичное восстановление AP с клампом к эффективному maxAp; без `amount` — полное восстановление. Учитывай, что реакции на смерть разрешаются до списания стоимости действия.
