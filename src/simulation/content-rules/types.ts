...
export type ContentRule = {
  ...
  /**
   * Опт-in «глобальной досягаемости»: правило собирается от всех живых акторов
   * независимо от дистанции до события (слой `global`, последний).
   * Предназначено для on-death/on-remove правил реликвий, реагирующих на события
   * вне радиуса владельца. Обязательно сочетать с условиями `eventRole`
   * и/или точными `hasStatus`/`hasTag` — иначе копия правила каждого владельца
   * будет срабатывать на каждое подходящее событие.
   */
  reach?: 'global';
};
