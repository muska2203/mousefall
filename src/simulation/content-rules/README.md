...
- `chance` — константа или параметризованная вероятность (использует `state.runtimeRng`, см. раздел ниже). **Внимание:** опциональная «рандомная» механика по выбору игрока; основные механики детерминированы. Использовать в новом контенте — только по явному указанию пользователя (решение 2026-08-04, `roadMap.md`, вопрос 1);
