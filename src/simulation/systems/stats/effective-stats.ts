...
// ─────────────────────────────────────────────
// Очки действий (акторы)
// ─────────────────────────────────────────────

/**
 * Итоговый максимум AP с учётом модификаторов (стат `maxAp`).
 * База — поле `maxAp` актора; итог округляется до целого и не ниже 0.
 * Читается при восстановлении AP и в снапшоте характеристик игрока.
 */
export function getEffectiveMaxAp(actor: StatActor & { maxAp: number }): number {
  return Math.round(applyModifiers(actor, 'maxAp', actor.maxAp).total);
}
