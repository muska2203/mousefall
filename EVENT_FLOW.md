...
Полный список событий: `src/simulation/types.ts` (union `GameEvent`).
...
│  UI Layer                               │
│                                         │
│  Исполняет AnimationPlan через PixiJS   │
...
Реализация этого перевода будет в `src/presentation/` (слой пока не реализован).
...
6. Presentation обходит дерево:
   ...
   - Формирует combatLog[]
   - Обновляет ViewModel
7. Presentation отдаёт UI: ViewModel + AnimationPlan + combatLog
...
Реализация будет в `src/presentation/combatLogBuilder.ts` (планируемый файл).
...
1. Добавить новый тип в union `GameEvent` в `src/simulation/types.ts`
