...
- Генератор размещает врагов, лут, двери, лестницы и **гарантированный poi стартовой комнаты** (`MapParams.startPoiId`, 8-соседняя со спавном клетка; сейчас — `relic_altar`, этап 0 roadmap). **Пропы, ловушки и не-floor террейн (песок) на этажах не спавнятся** — только через debug-действия или доработку генератора. В `MapParams` нет пулов пропов/ловушек/poi/террейнов (случайный спавн poi из пула не реализован).
