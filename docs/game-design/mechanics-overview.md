...
11 типов: `poisoned, burning, frozen, stunned, dazed, silenced, regenerating, counterattack, wet, oiled, bulwark`. Экземпляр: `duration` (в ходах фракции носителя), `value`, `stacks`, `statModifiers`. Тик — в начале хода фракции носителя (у дверей/пропов — в `environment-turn`).
...
Особые эффекты: `stunned` — пропуск хода, сбрасывает подготовленные скиллы; `silenced` — запрет способностей; `wet` снимает и несовместим с `burning`; `burning`↔`frozen` взаимоисключающи; `stunned` снимает `dazed`. Тики урона (яд 8% maxHp, горение 10% maxHp) заданы правилами — перенастраиваются контентом. `bulwark` («Глухая оборона» босса) — движковая семантика: полный иммунитет к урону (событие с damage 0 эмитится — статусы накладываются) и к толчкам, запрет всех действий носителя, кроме END_TURN, **без** сброса подготовленного скилла (отличие от `stunned`).
