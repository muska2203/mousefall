...
Фаза 4 выполнена на **100%** (6 из 6 шагов).
...
- ✅ Шаг 4.6 — Поддержка мультитаргетных селекторов (`allInRadius`, `nearestEnemy`); `chain` отложен

---

## ✅ Выполненные шаги

### Шаг 4.6. Поддержка мультитаргетных селекторов — `allInRadius` и `nearestEnemy`

**Статус:** завершён.

**Что сделано:**

- Селектор `chain` исключён из текущего шага и отложен до появления скилла «цепная молния».
- В `src/simulation/content-rules/types.ts` у `allInRadius` добавлено опциональное поле `excludeSelf?: boolean`.
- В `src/simulation/content-rules/reaction/content-rule-reaction.ts` доработан `resolveAllInRadius`:
  - исключает владельца при `excludeSelf === true`;
  - исключает мёртвых акторов (`isAlive === false`);
  - сортирует результат по `id` для детерминизма.
- `targetConditions` уже применялись per-candidate; добавлены unit-тесты на `hasStatus` с `subject: 'candidate'`.
- Добавлены unit-тесты:
  - `allInRadius` включает владельца без `excludeSelf` и исключает при `excludeSelf: true`;
  - `allInRadius` с `faction: 'enemy'` не затрагивает владельца;
  - мёртвые акторы не попадают в выборку;
  - порядок целей стабилен при перестановке `state.entities`.
...
- Создан `src/simulation/content-rules/world-rules/global-rules.ts` с `GLOBAL_WORLD_CONTENT_RULES`.
- `src/simulation/content-rules/rules.ts` стал тонким посредником:
  - реэкспортирует `WORLD_CONTENT_RULES` из `world-rules/global-rules.ts`;
  - сохраняет `getWorldContentRules` и `setWorldContentRulesOverride`.
- В `src/simulation/content-rules/reaction/content-rule-reaction.ts`:
  - добавлено поле `worldLayer` в `LayeredRule`;
  - в `collectRules` заполняется `worldLayer` для правил слоя `world`;
  - добавлен `WORLD_LAYER_ORDER` (`global: 0`, `tileEffect: 1`, `tileIntrinsic: 2`);
  - сортировка в слое `world` идёт по `worldLayer`, затем `priority`, затем `ruleId`.
- Добавлены unit-тесты:
  - порядок `global → tileEffect → tileIntrinsic` независимо от `priority`;
  - tie-break по `ruleId` при равных `worldLayer` и `priority`.
...
- `src/simulation/systems/starting-equipment.ts` переведён на стандартные интенты:
  - создание экземпляров предметов и `inventory.push` оставлены;
  - прямая мутация `equipped*Id/equipped*InstanceId`, `addModifier` и `player.abilities.push` убраны;
  - для каждого слotted-предмета выполняются `EQUIP_ITEM` и `GRANT_ABILITY` через локальный `ExecutionBuilder`;
  - финальный `recalculateActorStats` убран, так как `executeEquipItemIntent` уже пересчитывает статы.
- `src/simulation/systems/rules/active-rule-lifecycle.ts` (`rebuildActiveRules`):
  - устранено дублирование правил экипировки у игрока: если у актора есть `inventory`, используется только instance-based путь, иначе — template-based путь для врагов;
  - логика статусов и способностей не изменена.
- Добавлена тестовая инфраструктура для source-bound правил:
  - `src/simulation/content-rules/registry.ts` — `setContentRulesOverride` и merged-view `getRegistry`/`getAllContentRules`;
  - `tests/fixtures/content-rules.ts` — хелпер `withContentRules` и тестовые правила `testStatusRestoreAp`, `testAbilityFireMultiplier`.

**Тесты:**

- `tests/integration/content-rule-sources.test.ts` — три интеграционных теста:
  1. правило предмета (`slashing_weapon_bleed`) активируется при стартовой экипировке и накладывает `poisoned` при рубящем уроне;
  2. правило статуса (`status_restore_ap_on_damage`) восстанавливает AP владельцу при получении урона;
  3. правило способности-таланта (`ability_fire_multiplier`) удваивает огненный урон.
- `tests/unit/simulation/rules/active-rule-lifecycle.test.ts` — добавлен тест, что `rebuildActiveRules` не дублирует правила у игрока, у которого одновременно есть `inventory` с equipped-instance и `equippedWeaponId`.
...
|---|---|---|
| `npm test` | ✅ 122 файла, 976 тестов | 2026-07-15 |
| `npm run typecheck` | ✅ успешно | 2026-07-15 |
| `npm run build` | ✅ успешно | 2026-07-15 |

---

## Следующее действие

Перейти к **фазе 5 — Переключение**.
