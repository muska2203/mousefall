...
> **Примечание:** примеры с тайловыми эффектами (`TILE_EFFECT_CHANGED`, `EXPLOSION_TRIGGERED` и др.) относятся к фазе 7 «Тайловые эффекты и окружение», которая вынесена за пределы MVP. Концепт отображения цепочек сохраняется в неизменном виде.
