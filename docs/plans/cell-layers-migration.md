...
3. **Учёт эффектов клетки в AI/pathfinding**: взвешенный `findPath` (`src/utils/math.ts:167`), опасные клетки для врагов. Отправная точка — зафиксированное в фазе 1 ограничение: автопуть и AI-pathfinding равностоимостны, `moveCost` террейна учитывается только при списании AP за одиночный шаг (`DefaultActionPointCostResolver`), но не при выборе маршрута (см. `docs/architecture/OVERVIEW.md`). После фазы 5 сюда же относится уклонение врагов от раскрытых ловушек: скрытые ловушки сознательно не влияют на pathfinding (проверено тестом автопути), а раскрытые (`hidden = false`) AI пока игнорирует — как дым в фазе 3.
...
| 5. Ловушки | ✅ | Реализована 2026-07-30; trap (hidden, oneShot в шаблоне, слот floorFixture), слой `object` собирает правила ловушек независимо от hidden, lifecycle процедурный: срабатывание правила ловушки порождает интенты DESTROY_OBJECT (одноразовая) / REVEAL_OBJECT (постоянная) — решение в начале фазы: выделенные интенты + исполнители, без нового RuleEffect (по образцу процедурных charges у poi); пример: колючки (урон 10, одноразовые; поиск и обезвреживание отложены) |
