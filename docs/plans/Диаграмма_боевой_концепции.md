...
flowchart TD
    ...
    Event --> RuleContext["RuleContext\nsource / target / position / tags"]
    RuleContext --> ContentReactions["ContentRuleReaction\n(контентные реакции)"]
    ...
    ActiveRules["activeRules[]\n+ ownerContext"] -.->|source / target / world / radius| Modifiers
...
4. Из события строится **RuleContext** (`sourceEntityId`, `targetEntityId`, `eventPosition`, `eventTags` и т.д.).
5. Сначала срабатывает **ContentRuleReaction** (контентные реакции) на основе `RuleContext`, затем — **системные мировые реакции**. Обе могут породить новые интенты.
6. Новые интенты снова проходят через модификаторы → исполнитель → реакции, пока не сработает ограничение на количество реакций.
...
- **Контекст правил (`RuleContext`)** — введён явный контекст для `ContentRuleReaction`: `sourceEntityId`, `targetEntityId`, `eventPosition`, `eventTags` и специфичные поля (`collisionTargetId`, `abilityTargetPosition`).
- **Жизненный цикл правил и `activeRules`** — правила кэшируются у актора в `activeRules` с `ownerContext`; добавление/удаление при экипировке, статусах и талантах; подробности в [`Жизненный_цикл_правил_и_activeRules.md`](./Жизненный_цикл_правил_и_activeRules.md).
