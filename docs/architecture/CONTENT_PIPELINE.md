...
- **Entity Template** — поля: id, symbol, health, baseStats, attack, armor, modifiers, ai, lootTable, interactionKind и др. (урон, дальность и теги атаки врага — в профиле `attack`, броня — в поле `armor`, фирменные свойства — через `modifiers`; экипировки у врагов нет).
...
Контентные правила (content rules) — это data-driven способ описывать **реакции** на игровые события и **модификаторы** интентов (например, модификаторы урона). Они хранятся отдельно от шаблонов, но шаблоны способностей, статусов, террейнов, тайловых эффектов, POI, ловушек и реликвий ссылаются на них по `ruleIds`; предметы получают правила опосредованно — через модификаторы (`fixedModifiers`/аффиксы с `effect.kind: 'rule'`).
...
Шаблоны способностей, статусов и прочих категорий, перечисленных выше, содержат поле `ruleIds` — массив строковых идентификаторов правил (у предметов этого поля нет с 2026-08-09: их правила приходят через модификаторы экипировки). При создании экземпляра актора кэшируются активные правила (`activeRules`) из экипировки, статусов и т.д. Этот кэш используется системами реакций и модификаторов.
