...
1. **Создай TS-шаблон** в `src/content/templates/items/consumables/my-consumable.ts`. Имя файла — `id` в kebab-case, константа — camelCase:
   ...
   - `stackable`, `maxStack` — можно ли складывать и максимальный размер стопки. При подборе стакаемый предмет сливается в первый неполный стек того же шаблона до `maxStack`, остаток кладётся новой ячейкой. `stackable: true` несовместим с `abilityPool` (экземпляры с ролленными способностями не сливаются) — проверяется Zod-схемой.
   - `value` — цена продажи.
