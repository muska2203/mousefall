...
- Для **нового экземпляра существующего вида** (`selfBuff`, `swoop`, `groundSlam`, `fireball`, `magicSlap`, `dash`, `suddenStrike`, `cleave`, `throw`, `search`) — только шаблон и тексты: исполнитель собирается фабрикой из параметров шаблона.
...
1. **Выбери `kind`** — вид механики способности (дискриминатор union, camelCase):
   - **существующий вид** (`selfBuff`, `swoop`, `groundSlam`, `fireball`, `magicSlap`, `dash`, `suddenStrike`, `cleave`, `throw`, `search`) — параметры механики задаются в шаблоне, исполнитель соберётся фабрикой автоматически;
...
2. **Создай TS-шаблон** в `src/content/templates/abilities/my-ability.ts`. Имя файла — `id` в kebab-case, константа — camelCase:
   ...
   - `kind: 'search'` — `radius` (радиус поиска вокруг кастера, ≥ 1, Чебышёв): раскрывает скрытые ловушки в прямой видимости. Урона нет; targetMode `self` (применяется мгновенно). Контентный пример — `search` (врождённая способность игрока, выдаётся через `innateAbilities` шаблона игрока).
