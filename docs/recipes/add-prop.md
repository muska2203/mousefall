...
Отдельного рецепта для дверей пока нет; двери — своя категория (`src/content/templates/doors/`,
тексты — `environment.ts`, спрайты — `public/assets/objects/doors/`). Поля, появившиеся с
инфраструктурой босс-файта (2026-08-14):

- `indestructible` (default `false`) — неразрушаемая дверь: движок обнуляет любой урон
  по ней. Пример: `boss_door` (тег `boss_room`, негорючая).
- `isLocked` — не поле шаблона, а состояние экземпляра `DoorEntity`: запертую дверь
  нельзя открыть, она непроходима для AI/автопути/рывка; управляется интентами
  `LOCK_DOOR`/`UNLOCK_DOOR` (см. `src/simulation/AGENTS.md`, раздел про двери).
