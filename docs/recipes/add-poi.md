...
Гарантированные poi размещаются генератором этажа через `fill.guaranteedPois` типа комнаты
(категория `roomTypes`; стартовой комнате назначается тип `MapParams.startRoomTypeId` — см.
`docs/recipes/add-room-type.md`). Случайного спавна poi из пула нет.
