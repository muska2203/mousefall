...
1. **Создай шаблон материала** в `src/content/templates/tile-effects/oil.ts`. Имя файла — `id` в kebab-case, константа — camelCase:
   ...
   - `placement` — опциональное переопределение размещения спрайта эффекта в клетке
     (`scale`/`anchorX`/`anchorY`/`flattenY`). Дефолт — по слою: `cover` лежит в плоскости пола,
     `aboveGround` стоит в полный размер; см. `src/presentation/spritePlacementResolver.ts`.
...
2. **Если материал может менять поведение**, создай статус тайлового эффекта в `src/content/templates/tile-effect-statuses/burning.ts`:
   ...
   - `placement` — опциональное переопределение размещения значка статуса
     (дефолт: масштаб 0.7, низ на 0.5 высоты клетки, по центру; см. `src/presentation/spritePlacementResolver.ts`).
