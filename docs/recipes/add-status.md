...
1. **Создай TS-шаблон** в `src/content/templates/statuses/my-status.ts`. Имя файла — `id` в kebab-case, константа — camelCase:
   ...
   - `statusCategory` — категория для разрешения конфликтов (`elemental`/`physical`/`mental`/`poison`/`wound`/`control`/`generic`). В одном батче на сущность накладывается не более одного статуса каждой категории — статусы, которые один источник накладывает одновременно (например, ловушка: bleeding + rooted), обязаны быть в разных категориях.
   ...
   - `statModifiers` — модификаторы характеристик (`{stat, value, op}`), действующие, пока статус активен: применяются при наложении и снимаются при любом удалении статуса (expire, вытеснение, стеки → 0, конец `stunned`). Пример — шаблон `dazed` (`maxAp −1`).
