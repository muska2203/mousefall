...
PLAYER: ACTION_APPLIED (действие игрока) + FOV-обновление
  │
  ▼
AI: NOTIFY_AI → AI_NOTIFIED (реакции врагов на движение/двери)
  │
...
Во время хода игрока определённые доменные события порождают мировые реакции, которые уведомляют AI-стратегии об изменениях мира. Это позволяет врагам реагировать на движение игрока, открытие/закрытие дверей и другие заметные события до начала хода окружения.
...
> Стратегия может менять своё внутреннее состояние вне своего хода, но не может выполнять действия. Действия выполняются только в фазе `ENVIRONMENT` через `decideAction`.
