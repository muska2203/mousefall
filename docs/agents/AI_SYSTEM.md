...
> **Статус:** `[DRAFT]` — AI активно дорабатывается; документация может отставать от кода.
> **Источник правды:** `src/simulation/ai/`.
...
детерминированном порядке (расстояние до цели, x, y); клетки, занятые живыми
акторами, пропускаются — там механика подставки (жертву отпихивает на ближайшую
свободную клетку, столкновения с препятствием не будет, а кастер может ударить
союзника); цель должна попадать в
