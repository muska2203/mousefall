...
```json
{
  ...
  "type": "weapon",
  "ruleIds": ["weapon_fire_damage_boost"],
  "weapon": {
    "baseDamage": 5,
...
Здесь оружие ссылается на правило `weapon_fire_damage_boost`, которое умножает огненный урон от оружного доставщика на 1.15. Такой же подход работает для статусов и способностей: шаблон указывает `ruleIds`, а реестр разрешает их в объекты правил.
