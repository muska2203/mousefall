...
Правила слоя `object` собираются из `ruleIds` шаблонов объектов (не акторов) на клетке события: точек интереса (poi), в будущем — ловушек и других объектов с `ruleIds`. `ownerContext` такого правила — `{ type: 'object', position, entityId }`, `selfId` — ID сущности-объекта (селектор `self` целится в сам объект).
