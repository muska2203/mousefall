...
> Этот документ собирает пограничные ситуации, тонкости порядка исполнения и рекомендации по отладке декларативных контентных правил. Ориентирован на разработчиков/агентов, которые будут добавлять новые правила.
...
4. [Порядок смерти (модель 1)](#порядок-смерти-модель-1)
5. [Конфликты статусов](#конфликты-статусов)
6. [Пустые селекторы](#пустые-селекторы)
7. [Порядок слоёв](#порядок-слоёв)
8. [Параметризованные значения](#параметризованные-значения)
9. [Модификаторы на интенте](#модификаторы-на-интенте)
10. [Атрибуция убийства (правила «при убийстве»)](#атрибуция-убийства-правила-при-убийстве)
11. [Отладка](#отладка)
...
Ограничение касается только реакций **на то же событие** (снимок правил в начале фазы). Уже следующая волна интентов видит наложенное полностью: и `hasStatus` на носителе, и правила из его обновлённых `activeRules`. В частности, `ENTITY_DIED` — событие следующей волны (см. «Порядок смерти (модель 1)»), поэтому реакции на смерть видят статусы, наложенные смертельным ударом.
...
## Порядок смерти (модель 1)

> Утверждено в `docs/plans/bleed-builds-implementation.md` (§1, п. 2); зафиксировано тестами `tests/unit/simulation/content-rules/death-order-model.test.ts`.

### Правило

Смерть — **последнее**, что происходит в волне реакций на смертельный урон:

```text
DAMAGE intent
  → executeIntent → ENTITY_DAMAGED
    → контентная фаза волны: on-hit правила вешают статусы (APPLY_STATUS)
    → deathReaction (world-reaction) порождает DIE
  → следующая волна: APPLY_STATUS исполняется, executeDieIntent выставляет
    isAlive = false и эмитит ENTITY_DIED
    → реакции на ENTITY_DIED видят финальное состояние цели
```

### Следствия

- **Реакции на смерть видят статусы, наложенные смертельным ударом.** Один удар может наложить статус, убить и вызвать on-death реакцию на этот статус (условие `hasStatus ... subject: 'target'` на `ENTITY_DIED` проходит). Прецедент — детонация бочек с `hasStatus burning` на трупе; то же свойство используют on-death правила кровавой ветки («Разрыватель» детонирует и от ваншота).
- **Смерть не порождает `STATUS_REMOVED`.** Статусы остаются на трупе до удаления сущности в конце раунда (`CLEANUP_DEAD_ENTITIES`). Поэтому добивание кровоточащего не считается «спаданием» кровотечения: правила на `STATUS_REMOVED` срабатывают только на естественное спадание, вытеснение и стеки → 0 (минус «Кровавого эха», решение §1 п. 3 того же плана).
- **Труп не актор для сбора правил.** Слой `radius` (и селекторы целей) работают только по живым акторам (`isAlive === true`), поэтому `activeRules` трупа не подхватываются последующими событиями. При этом `hasStatus` на трупе как на **цели** события работает — см. первый пункт.
...
4. При равных приоритетах используется порядок правил: `source` → `target` → `world` → `radius` → `global`, затем `priority`, затем `ruleId`.
...
```text
source → target → world → radius → global
```

### Сортировка внутри слоя

1. **world** сначала сортируется по подтипу: `global` → `tileEffect` → `tileEffectStatus` → `object` → `tileIntrinsic`.
...
Правила слоя `object` собираются из `ruleIds` шаблонов объектов (не акторов) на клетке события: точек интереса (poi) и ловушек (trap), в будущем — других объектов с `ruleIds`. `ownerContext` такого правила — `{ type: 'object', position, entityId }`, `selfId` — ID сущности-объекта (селектор `self` целится в сам объект).

Разовость эффектов объектов (например, одноразовый алтарь) обеспечивается **процедурно** — исполнитель `ACTIVATE_POI` проверяет `charges > 0` и декрементит заряды, а правила остаются декларативными по эффекту и срабатывают на событие `POI_USED`.

#### Ловушки (trap)

- Правила ловушек собираются **независимо от `hidden`**: скрытая ловушка срабатывает (обычный триггер — `ENTITY_MOVED` на клетку с ловушкой, цель — `eventSource`, т.е. вошедший игрок или враг).
- Жизненный цикл процедурный: срабатывание правила, владелец которого — ловушка, порождает lifecycle-интенты в `runContentRuleReactions`. Одноразовая ловушка (`oneShot: true`) уничтожается интентом `DESTROY_OBJECT` (событие `OBJECT_DESTROYED`), постоянная раскрывается интентом `REVEAL_OBJECT` (`hidden = false`, событие `OBJECT_REVEALED`) и может срабатывать повторно.
- Lifecycle-интент порождается **на каждое сработавшее правило** ловушки: у oneShot-ловушки с тремя правилами (мышеловка, 2026-08-23) эмитятся три `DESTROY_OBJECT`. Дубликаты безвредны — исполнитель возвращает `null` для уже уничтоженной ловушки, событие `OBJECT_DESTROYED` остаётся одним (см. `tests/unit/simulation/content-rules/mousetrap.test.ts`).
- Порядок слоёв не менялся: правила ловушки идут в слое `object`, то есть после `tileEffect`/`tileEffectStatus`. Если клетка одновременно горит и содержит ловушку, сначала срабатывают правила тайлового эффекта, затем правило ловушки — порядок детерминирован.
...
`global` — правила с опт-in полем `reach: 'global'`, собранные от **всех живых акторов** независимо от дистанции до события (и от наличия `eventPosition`). Последний слой; дедупликация по `(rule.id, selfId)` против уже собранных копий из слоёв `source`/`target`/`radius` — владелец, попавший в один из этих слоёв, не исполняет правило повторно. Трупы (`isAlive === false`) не собираются, как и в `radius`.

Не путать с **подтипом** `worldLayer: 'global'` слоя `world` (мировые правила без владельца, `GLOBAL_WORLD_CONTENT_RULES`): совпадение названия историческое, механизмы разные — там реестр мировых правил, здесь `activeRules` акторов с флагом `reach`.

Предназначение — on-death/on-remove правила реликвий, реагирующие на события вне радиуса владельца (минусы «Жатвы» и «Кровавого эха», план `docs/plans/bleed-builds-implementation.md`, §1 п. 4).

> ⚠️ Предостережение слоя `radius` («правило владельца обязано иметь `eventRole`») для слоя `global` ещё сильнее: копия правила есть у **каждого** живого актора-владельца. Сочетайте `reach: 'global'` с `eventRole` и/или точными `hasStatus`/`hasTag`/`eventFieldEquals`, иначе правило сработает на каждое подходящее событие у каждого владельца.
>
> ⚠️ Слой `global` существует только в сборе реакций на события (`content-rule-reaction.ts`). Модификаторы на интенте (`apply-intent-modifiers.ts`, `modifyDamage`) слой `global` не собирают — `reach` там не действует.
...
> ⚠️ Реакции, отвечающие источнику урона (`target: { type: 'eventSource' }`, шипы и т.п.), обязаны учитывать самоурон: при self-hit владельца (например, собственный Налёт с тегом `attack.melee` — кастер задевал себя ударом по клетке приземления) правило безусловно сработает на самого владельца. Добавляйте условие `notSelfHit` — оно отсекает события с `source === target` (мировой урон без источника самоуроном не считается). Прецедент: `armor_spiked_thorns` (исправлено 2026-08-26); известный неисправленный аналог — `relic_acid_blood_poison_attacker` (тот же паттерн, осознанно оставлен).
...
| `notSelfHit` | Событие не является самоуроном: источник не задан или `source ≠ target`. |
...
## Атрибуция убийства (правила «при убийстве»)

### Откуда берётся убийца

Событие `ENTITY_DIED` несёт поле `sourceEntityId` — источник последнего урона
(`deathReaction` прокидывает его из `ENTITY_DAMAGED` через `DIE`-интент).
В `RuleContext` для `ENTITY_DIED` это поле попадает в `sourceEntityId`, поэтому:

- правила убийцы собираются слоем `source`;
- условие `eventRole: 'source'` отличает «убил владелец правила»;
- селектор `self` нацеливает эффект на убийцу (например, `restoreAp`/`heal`).

### Когда «при убийстве» НЕ срабатывает

- **Смерть без источника** (среда, мировой урон): `sourceEntityId === null` —
  слой `source` пуст, `eventRole: 'source'` не проходит.
- **Дот-килл** (тик яда/кровотечения): правило тика принадлежит жертве, поэтому
  источником DAMAGE-интента является сама жертва (`sourceEntityId === targetEntityId`).
  Чужие правила «при убийстве» не срабатывают — дот-киллы сознательно не
  засчитываются (решение 2026-08-27, roadmap-1-floor п. 1.3).
- Смерть игрока идёт через `PLAYER_DIED`, а не `ENTITY_DIED` — на неё
  правила «при убийстве» не подписываются.

### Порядок внутри действия

Реакции на `ENTITY_DIED` разрешаются до списания стоимости действия (AP тратится
в конце). Поэтому `restoreAp` с `amount` при полных AP в момент убийства даст
дельту 0, а списание стоимости атаки произойдёт уже после восстановления.
