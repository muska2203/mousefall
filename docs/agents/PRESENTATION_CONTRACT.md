...
> **Статус:** `[STABLE]` — контракт Simulation + Presentation устаканился.
...
Тип: `src/presentation/displayState/types.ts`.  
Построение и применение: `src/presentation/displayState/builder.ts`.
...
- [ ] Если builder порождает новый тип `AnimationNode`, добавить/обновить executor в `src/ui/animation/` (например, `src/ui/animation/spriteExecutor.ts`, `src/ui/animation/projectileExecutor.ts`).
...
Допустим, правило накладывает статус `electrified` (предварительно добавленный в `StatusEffectType`).

1. Событие `STATUS_APPLIED` уже существует в Simulation.
...
3. Для `STATUS_APPLIED` уже зарегистрирован `statusAppliedBuilder`, поэтому расширяем его, а не регистрируем новый builder:

```ts
// src/presentation/animation/builders/statusApplied.ts
import type { AnimationBuilder } from '../core/registry';
import { statusBurstForEntity, particleBurstNode } from '../core/primitives';
...
export const statusAppliedBuilder: AnimationBuilder = (event, children, state) => {
  ...
  const base = statusBurstForEntity(event.entityId, entity, event.effect.type, children);
  const baseNodes = base ? [base] : null;

  if (event.effect.type === 'electrified' && entity) {
    const spark = particleBurstNode({ x: entity.x, y: entity.y }, 0xffff00, 8, []);
    return baseNodes ? [spark, ...baseNodes] : [spark];
  }
...
> Примечание: если для события уже есть builder, повторная регистрация в `registerAnimationBuilder` затрёт предыдущий. Расширяйте существующий builder внутри его файла.

4. Добавить i18n-ключ `system.gameSession.effectElectrified` в `src/i18n/schema.ts` и оба locale-файла.
5. Добавить/обновить unit-тест в `tests/unit/presentation/animation/builders.test.ts` (или аналогичный).
